...
小孩常常當天忘記按「我完成了！」。**3 天內都可以補送出**：

- 首頁會出現「🕒 前幾天還沒完成的」，列出漏掉的任務與是幾天前的
- 行事曆點進那一天，也可以直接補按

補送出的任務走一樣的審核流程，家長確認後正常加點；稽核紀錄會標明是補送的
（例如「補送 09/07 的任務」），家長看得出來差別。

超過 3 天就不能自己補了 —— 否則「當下完成」的意義會消失，家長也很難判斷
到底有沒有真的做。這種情況請家長用「⭐ 點數 → 手動調整」處理。

補送天數定義在 `family_reward/services/assignment_service.py` 的 `MAKEUP_DAYS`。
...
開啟後按 <kbd>Ctrl</kbd>+<kbd>P</kbd>，「目的地」選**「另存為 PDF」**就能存檔，
或直接選印表機印出來。手機也可以。

獎狀內容：這個月得到幾顆星星（大字）、集點卡、連續達成天數、已獲得的成就、
最常完成的任務，以及一個**家長簽名欄**。

> **為什麼不做「下載 PDF」按鈕？**
>
> 伺服器端產生 PDF 需要 Chromium（703 MB），會違背「解壓縮就能跑」的承諾。
> 更關鍵的是實測發現：headless Chromium 產生 PDF 時**不會內嵌 emoji 字型**，
> 🐼 🏆 ⭐ 全部變成空白 —— 而這個系統的 emoji 是資料，不是裝飾。
>
> 用你自己的瀏覽器列印時，emoji 用系統的彩色字型，中文與版面都完美，
> 而且零新依賴。代價只是多按一次 Ctrl+P。
...
| 檔案 | 內容 |
|---|---|
| 點數紀錄 | 每一筆加點與扣點，可以用 Excel 加總 |
| 任務完成明細 | 哪一天、哪個任務、狀態、拿到幾點 |
| 兌換紀錄 | 何時換了什麼、花了幾點 |

CSV 使用 **UTF-8 含 BOM**，Excel 可以直接開，中文不會亂碼。

### 全部資料 ZIP（搬家用）

**後台 → 📤 匯出 → 下載 ZIP**：一次打包所有月份的 CSV 加一份說明檔。
適合換電腦、搬到伺服器，或只是想留一份看得懂的資料在手上。
...
**別把 CSV 當成備份。** 要防資料遺失請用 `backup.bat`。
